          ...
          <div className="space-y-4">
            <div 
              ...
              className={`border-2 border-dashed rounded-xl h-64 flex flex-col items-center justify-center cursor-pointer transition-colors relative overflow-hidden group ${
                image ? 'border-sky-300 bg-sky-50' : 'border-slate-300 hover:border-sky-400 hover:bg-slate-50'
              }`}
            >
              <input 
                type="file" 
              ...
              {image ? (
                ...
                <div className="text-center p-6">
                  ...
                  <p className="text-xs text-slate-400 mt-1">XRD Patterns, HighScore Screenshots, Data Tables</p>
                </div>
              )}
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full h-24 px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 outline-none transition-colors text-sm"
              />
            </div>
            ...
            <button
              ...
              className={`w-full py-3 text-white font-medium rounded-lg shadow-md transition-all flex justify-center items-center gap-2
                ${(!image || loading) ? 'bg-sky-300 cursor-not-allowed' : 'bg-sky-600 hover:bg-sky-700 hover:shadow-lg active:scale-[0.98]'}
              `}
            >
              {loading ? (
                <>
      ...
      <div className="lg:col-span-7">
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 h-full min-h-[500px] flex flex-col">
          <div className="p-5 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
            ...
            {result && (
              <span className="text-xs bg-sky-100 text-sky-800 px-2 py-1 rounded-full font-bold">
                Done
              </span>
            )}
          </div>
          ...
          <div className="p-6 flex-1 overflow-y-auto custom-scrollbar">
            {!result ? (
              ...
              <div className="prose prose-sm prose-slate max-w-none">
                {/* 
                  Using simple whitespace-pre-wrap as a fallback for markdown structure 
                  since we aren't importing a heavy markdown renderer, 
                  or we could use ReactMarkdown if available in the ecosystem.
                  The user file list shows we don't have 'react-markdown' in importmap.
                  I will use basic formatting.
                */}
