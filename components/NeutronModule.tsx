...
import { NeutronAtom, NeutronResult } from '../types';
...
export const NeutronModule: React.FC = () => {
  const [wavelength, setWavelength] = useState<number>(1.54); // Common Neutron wavelength approx
  const [latticeA, setLatticeA] = useState<number>(4.20); // e.g. MgO is 4.21, H2O ice is different but let's use cubic for demo
  
  // Default: Magnesium Oxide (MgO) - Rock salt structure
  ...
  const updateAtom = (id: string, field: keyof NeutronAtom, value: any) => {
    setAtoms(atoms.map(a => {
      if (a.id === id) {
        ...
        // Auto-update b value if element changes
        if (field === 'element') {
           if (NEUTRON_SCATTERING_LENGTHS[value]) {
                ...
                {atoms.map((atom) => (
                  <div key={atom.id} className="bg-slate-50 p-2 rounded border border-slate-200 text-xs">
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      <input 
                        type="number" placeholder="x" step="0.1" value={atom.x}
                        onChange={(e) => updateAtom(atom.id, 'x', parseFloat(e.target.value))}
                        className="w-full px-1 py-1 bg-slate-900 text-white border border-slate-700 rounded font-mono font-bold"
                      />
                      <input 
                        type="number" placeholder="y" step="0.1" value={atom.y}
                        onChange={(e) => updateAtom(atom.id, 'y', parseFloat(e.target.value))}
                        className="w-full px-1 py-1 bg-slate-900 text-white border border-slate-700 rounded font-mono font-bold"
                      />
                      <input 
                        type="number" placeholder="z" step="0.1" value={atom.z}
                        onChange={(e) => updateAtom(atom.id, 'z', parseFloat(e.target.value))}
                        className="w-full px-1 py-1 bg-slate-900 text-white border border-slate-700 rounded font-mono font-bold"
                      />
                      <input 
                        type="number" placeholder="B" step="0.1" value={atom.B_iso}
                        onChange={(e) => updateAtom(atom.id, 'B_iso', parseFloat(e.target.value))}
                        className="w-full px-1 py-1 bg-slate-900 text-white border border-slate-700 rounded font-mono font-bold"
                      />
                    </div>
                  </div>
