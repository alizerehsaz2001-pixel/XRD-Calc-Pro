...
/**
 * Nelson-Riley Lattice Parameter Extrapolation Function
 * f(theta) = (cos^2(theta) / sin(theta)) + (cos^2(theta) / theta)
 * Linear extrapolation of apparent lattice constant a(hkl) to f(theta) -> 0
 * yields true zero-systematic-error lattice parameter a0!
 */
export const computeNelsonRileyFit = (
  indexedPeaks: IndexedPeakMatch[],
  referenceA0: number = 4.0 // default reference lattice parameter in Angstrom
): NelsonRileyFitResult | null => {
  const validPoints: NelsonRileyPoint[] = [];

  for (const p of indexedPeaks) {
    if (!p.twoThetaA || p.twoThetaA <= 5) continue;
    const thetaDeg = p.twoThetaA / 2;
    const thetaRad = (thetaDeg * Math.PI) / 180;
    const sinTheta = Math.sin(thetaRad);
    const cosTheta = Math.cos(thetaRad);

    if (sinTheta <= 0 || thetaRad <= 0) continue;

    // Nelson-Riley function
    const fnr = (cosTheta * cosTheta / sinTheta) + (cosTheta * cosTheta / thetaRad);
    const dVal = parseFloat(p.dSpacingA);
    if (isNaN(dVal) || dVal <= 0) continue;

    // Parse Miller indices if available (e.g. "111", "200", "220", "311", "222")
    let h = 1, k = 1, l = 1;
    const hklStr = (p.hklA || p.hklB || '').replace(/[^0-9-]/g, '');
    if (hklStr.length === 3) {
      h = parseInt(hklStr[0], 10) || 1;
      k = parseInt(hklStr[1], 10) || 1;
      l = parseInt(hklStr[2], 10) || 1;
    } else {
      // Fallback: estimate from d-spacing order
      const q = (2 * Math.PI) / dVal;
      const approxHklSq = Math.round(Math.pow((q * referenceA0) / (2 * Math.PI), 2));
      h = Math.max(1, Math.round(Math.sqrt(approxHklSq / 3)));
      k = h;
      l = h;
    }

    const hklFactor = Math.sqrt(h * h + k * k + l * l);
    const apparentA = dVal * (hklFactor > 0 ? hklFactor : 1);

    validPoints.push({
      hkl: p.hklA || p.hklB || `(${h}${k}${l})`,
      twoTheta: p.twoThetaA,
      dSpacing: dVal,
      fnr: Number(fnr.toFixed(4)),
      apparentA: Number(apparentA.toFixed(5)),
      residual: 0
    });
  }
  ...
  // Linear Regression: apparentA = a0 + slope * fnr
  const n = validPoints.length;
  let sumX = 0, sumY = 0, sumXY = 0, sumXX = 0;
  for (const pt of validPoints) {
    sumX += pt.fnr;
    sumY += pt.apparentA;
    sumXY += pt.fnr * pt.apparentA;
    sumXX += pt.fnr * pt.fnr;
  }

  const denom = (n * sumXX - sumX * sumX);
  const slope = Math.abs(denom) > 1e-9 ? (n * sumXY - sumX * sumY) / denom : 0;
  const a0 = Math.abs(denom) > 1e-9 ? (sumY - slope * sumX) / n : (sumY / n);

  // R-squared and residuals
  const meanY = sumY / n;
  let ssTot = 0, ssRes = 0;
  for (const pt of validPoints) {
    const yPred = a0 + slope * pt.fnr;
    pt.residual = Number((pt.apparentA - yPred).toFixed(5));
    ssTot += Math.pow(pt.apparentA - meanY, 2);
    ssRes += Math.pow(pt.apparentA - yPred, 2);
  }

  const rSquared = ssTot > 0 ? Math.max(0, 1 - (ssRes / ssTot)) : 1.0;
  const stdErrA0 = Math.sqrt(Math.max(0, ssRes / Math.max(1, n - 2)) / n);

  const deltaA = a0 - referenceA0;
  const strainPercent = referenceA0 > 0 ? (deltaA / referenceA0) * 100 : 0;
  const unitCellVolume = Math.pow(a0, 3);
  const refVolume = Math.pow(referenceA0, 3);
  const deltaVolumePercent = refVolume > 0 ? ((unitCellVolume - refVolume) / refVolume) * 100 : 0;

  return {
    a0: Number(a0.toFixed(5)),
    stdErrA0: Number(stdErrA0.toFixed(5)),
    slope: Number(slope.toFixed(6)),
    rSquared: Number(rSquared.toFixed(4)),
    points: validPoints,
    deltaA: Number(deltaA.toFixed(5)),
    strainPercent: Number(strainPercent.toFixed(4)),
    unitCellVolume: Number(unitCellVolume.toFixed(3)),
    deltaVolumePercent: Number(deltaVolumePercent.toFixed(3))
  };
};

/**
 * Extracts positive residual peaks (I_exp - I_model > threshold) to identify secondary/impurity phases
 */
export const extractPositiveResidualPeaks = (
  points: ProfilePoint[],
  minThreshold = 6.0
): PeakItem[] => {
  if (!points || points.length < 5) return [];

  const rawResid = points.map(p => ({
    twoTheta: p.twoTheta,
    intensity: Math.max(0, p.difference)
  }));

  return extractPeaksFromRawPoints(rawResid.filter(p => p.intensity >= minThreshold));
};

/**
 * Generates a high-precision, publication-grade Markdown Scientific Report
 */
export const generateMarkdownReport = (
  ...
  indexedPeaks: IndexedPeakMatch[],
  phaseFractions: { fracB: number; fracC: number; fracD: number }
): string => {
  const timestamp = new Date().toUTCString();
  let md = `# XRD DIFFRACTION COMPARISON & MICROSTRUCTURAL REPORT\n\n`;
  md += `**Generated:** ${timestamp}  \n`;
  md += `**Instrument Standard:** Cu Kα Radiation (λ = 1.5406 Å)  \n\n`;

  md += `## 1. Specimen & Phase Configuration\n\n`;
  md += `| Role | Phase Name | Chemical Formula | Space Group / Type |\n`;
  md += `| :--- | :--- | :--- | :--- |\n`;
  md += `| **Sample A (Exp)** | ${materialA.name} | \`${materialA.formula || '—'}\` | ${materialA.crystalSystem || 'Experimental'} |\n`;
  md += `| **Reference B (Standard)** | ${materialB.name} | \`${materialB.formula || '—'}\` | ${materialB.crystalSystem || 'Standard'} |\n`;
  if (materialC) {
    md += `| **Phase C (Secondary)** | ${materialC.name} | \`${materialC.formula || '—'}\` | ${materialC.crystalSystem || 'Secondary'} |\n`;
  }
  if (materialD) {
    md += `| **Phase D (Tertiary)** | ${materialD.name} | \`${materialD.formula || '—'}\` | ${materialD.crystalSystem || 'Tertiary'} |\n`;
  }
  md += `\n`;

  md += `## 2. Statistical Goodness-of-Fit & Residuals\n\n`;
  md += `- **Profile Residual Factor ($R_p$):** \`${metrics.rP}%\`\n`;
  md += `- **Weighted Profile Residual ($R_{wp}$):** \`${metrics.rWP}%\`\n`;
  md += `- **Reduced Chi-Squared ($\\\\chi^2$ / Goodness of Fit):** \`${metrics.chiSquared}\`\n`;
  md += `- **Pearson Spectral Correlation ($r$):** \`${metrics.pearsonR}%\`\n`;
  md += `- **Figure of Merit (FOM):** \`${metrics.fom} / 100\`\n`;
  md += `- **Root Mean Square Deviation (RMSD):** \`${metrics.rmsd}\`\n\n`;

  md += `## 3. Quantitative Multi-Phase Composition (NNLS / RIR)\n\n`;
  md += `| Phase Component | Mass / Profile Fraction | Status |\n`;
  md += `| :--- | :---: | :--- |\n`;
  md += `| **${materialB.name}** | **${phaseFractions.fracB}%** | Primary Phase Matrix |\n`;
  if (materialC) {
    md += `| **${materialC.name}** | **${phaseFractions.fracC}%** | Secondary Phase |\n`;
  }
  if (materialD) {
    md += `| **${materialD.name}** | **${phaseFractions.fracD}%** | Tertiary Phase |\n`;
  }
  md += `\n`;

  md += `## 4. Indexed Reflections & Lattice Displacements Matrix\n\n`;
  md += `| Index | $2\\\\theta_{\\\\text{Exp}}$ (°) | $2\\\\theta_{\\\\text{Ref}}$ (°) | $(hkl)$ | $d_{\\\\text{Exp}}$ (Å) | $d_{\\\\text{Ref}}$ (Å) | $\\\\Delta 2\\\\theta$ (°) | Match Status |\n`;
  md += `| :---: | :---: | :---: | :---: | :---: | :---: | :---: | :--- |\n`;

  indexedPeaks.forEach(p => {
    md += `| ${p.id} | ${p.twoThetaA !== null ? `${p.twoThetaA.toFixed(2)}°` : '—'} | ${p.twoThetaB !== null ? `${p.twoThetaB.toFixed(2)}°` : '—'} | \`${p.hklA || p.hklB || '—'}\` | ${p.dSpacingA} | ${p.dSpacingB} | ${p.shift !== null ? `${p.shift > 0 ? `+${p.shift.toFixed(2)}` : p.shift.toFixed(2)}°` : '—'} | **${p.status.toUpperCase()}** |\n`;
  });

  md += `\n---\n*Report generated via XRD Diffraction Comparison & Crystallography Suite*\n`;
  return md;
};
