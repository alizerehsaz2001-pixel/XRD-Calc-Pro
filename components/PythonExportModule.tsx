...
export const PythonExportModule: React.FC = () => {
  ...
  const templates = [
    {
      id: "basic_analysis",
      label: "Basic Peak Analysis",
      category: "Analysis",
      icon: Terminal,
    },
    {
      id: "rietveld_refine",
      label: "Rietveld Automation",
      category: "Advanced",
      icon: Activity,
    },
    {
      id: "scherrer_batch",
      label: "Scherrer Particle Sizing",
      category: "Analysis",
      icon: Target,
    },
    {
      id: "thin_film",
      label: "Thin Film Texture",
      category: "Materials",
      icon: Layers,
    },
    {
  ...
  const handleAIGenerate = async () => {
    ...
    const peaksCount =
      (braggData?.rawPeaks || "").split(/[,;]+/).filter(Boolean).length || 3;
  ...
  const generateScript = () => {
    try {
      ...
      const phaseNames =
        phases.length > 0 ? phases.map((p: any) => p.name) : ["Phase1"];
...
`;
        ...
        phases.forEach((p: any, idx: number) => {
          ...
          const hklstr =
            p.crystalSystem === "Cubic"
              ? "xu.materials.Symmetry.Cubic()"
              : p.crystalSystem === "Tetragonal"
                ? "xu.materials.Symmetry.Tetragonal()"
                : p.crystalSystem === "Hexagonal"
                  ? "xu.materials.Symmetry.Hexagonal()"
                  : p.crystalSystem === "Orthorhombic"
                    ? "xu.materials.Symmetry.Orthorhombic()"
                    : "xu.materials.Symmetry.Triclinic()";
          xuMaterials += `
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-xl border border-slate-200 dark:border-white/5 relative overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8 relative z-10">
          <div className="flex items-center gap-4">
            <div className={`p-3 rounded-2xl shadow-xl flex items-center justify-center ${
              isAiMode ? 'bg-gradient-to-br from-fuchsia-500 to-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-100 border border-slate-200 dark:border-slate-700'
            }`}>
              {isAiMode ? <Sparkles className="w-6 h-6" /> : <Terminal className="w-6 h-6" />}
            </div>
            <div>
              <h2 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-indigo-500 to-cyan-500 dark:from-indigo-400 dark:to-cyan-400">
                {isAiMode
                  ? t("AI-Enhanced Python Lab", "AI-Enhanced Python Lab")
                  : t("Python Script Generator", "Python Script Generator")}
              </h2>
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                {isAiMode
                  ? "Gemini-designed custom analysis module tailored to your requirements."
                  : "Export computational configurations into an automated workflow."}
              </p>
            </div>
          </div>
        ...
        <div className="mb-8 relative z-10 p-[1px] bg-gradient-to-r from-slate-200/50 via-indigo-500/30 to-fuchsia-500/30 dark:from-slate-700/50 dark:via-indigo-500/50 dark:to-fuchsia-500/50 rounded-[2rem] shadow-lg overflow-hidden group/forge">
          <div className="bg-white dark:bg-[#0B0F19] rounded-[1.95rem] p-6 space-y-6">
            {/* TEMPLATE LIBRARY BAR */}
            <div className="pb-5 border-b border-slate-100 dark:border-slate-800/60">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-black uppercase text-slate-700 dark:text-slate-300 tracking-widest font-sans">
                    Script Template Library
                  </span>
                </div>
                <span className="text-[9px] font-bold text-slate-400 bg-slate-100 dark:bg-slate-800/50 px-2 py-1 rounded">
                  SELECT TEMPLATE
                </span>
              </div>
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 py-2 rounded-xl text-[11px] font-bold border transition-all ${
                      selectedTemplate === tpl.id && !isAiMode
                        ? "bg-indigo-600 text-white border-transparent shadow-lg shadow-indigo-500/25 scale-105"
                        : "bg-slate-50 dark:bg-slate-900/50 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-800 hover:border-indigo-500/50 hover:bg-white dark:hover:bg-slate-800"
                    }`}
                  >
                    <tpl.icon
                      size={14}
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-fuchsia-500 to-fuchsia-700 flex items-center justify-center text-white shadow-lg shadow-fuchsia-500/30 group-hover/forge:scale-105 transition-transform">
                  <Brain size={24} />
                </div>
                <div>
                  <h3 className="text-lg font-black text-slate-800 dark:text-slate-100 flex items-center gap-2">
                    Gemini AI Neural Forge
                    <span className="text-[10px] bg-fuchsia-100 dark:bg-fuchsia-500/20 text-fuchsia-700 dark:text-fuchsia-400 px-2.5 py-0.5 rounded-full uppercase tracking-wider font-bold border border-fuchsia-200 dark:border-fuchsia-500/30 animate-pulse">
                      Intelligent
                    </span>
                  </h3>
                  <p className="text-xs text-slate-500 font-medium">
                    Describe your requirements to synthesize custom python analysis code
                  </p>
                </div>
              </div>
              <div className="hidden lg:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800">
            ...
            <div className="relative group/textarea">
              ...
              <textarea
                ...
                className="w-full bg-slate-50 dark:bg-[#070A11] border-2 border-slate-200 dark:border-slate-800 rounded-2xl pl-12 pr-4 py-4 text-sm focus:ring-4 focus:ring-fuchsia-500/10 focus:border-fuchsia-500/50 outline-none transition-all min-h-[120px] text-slate-800 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-600 resize-y"
              />
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button
                ...
                className={`w-full sm:w-auto px-6 py-4 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-3 transition-all shadow-xl active:scale-95 shrink-0 ${
                  isGenerating
                    ? "bg-slate-200 dark:bg-slate-800 text-slate-400 cursor-not-allowed"
                    : "bg-gradient-to-r from-fuchsia-600 to-indigo-600 hover:from-fuchsia-500 hover:to-indigo-500 text-white shadow-indigo-500/25"
                }`}
              >
                {isGenerating ? (
                  <>
                    <Activity size={18} className="animate-spin" /> Coding...
                  </>
                ) : (
                  <>
                    <Zap size={18} /> Generate Script
                  </>
                )}
              </button>
              ...
              <div className="flex-1 flex flex-wrap gap-2 content-start">
                {(aiSuggestionsByLibrary[selectedLibrary] || []).map((s, i) => (
                  <button
                    ...
                    className="px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-[10px] font-bold text-slate-600 dark:text-slate-400 hover:text-fuchsia-600 dark:hover:text-fuchsia-400 hover:bg-white dark:hover:bg-slate-800 hover:border-fuchsia-500/30 transition-all text-left flex-1 min-w-[200px]"
                  >
                    {s}
                  </button>
                ))}
                ...
                <div className="space-y-1">
                  {neuralLogs.map((log, idx) => (
                    <div key={idx} className="flex items-start gap-2">
                      ...
                      <span
                        className={`${idx === neuralLogs.length - 1 ? "text-white font-extrabold animate-pulse" : "opacity-80"}`}
                      >
                        {log}
                      </span>
                    </div>
        ...
        <div className="relative group z-10 flex flex-col mt-4">
          <div className="flex items-center justify-between bg-slate-900 border-t border-x border-slate-800 rounded-t-xl px-4 py-3">
            {isAiMode ? (
              <div className="flex items-center gap-2">
                <Brain className="w-4 h-4 text-fuchsia-400 animate-pulse" />
                <span className="text-xs uppercase tracking-wider font-bold text-fuchsia-200">
                  AI Synthesis Active
                </span>
                {userEdited && (
            ...
            <div className="flex gap-2">
              <button
                onClick={handleCopy}
                className="bg-slate-800 hover:bg-slate-700 text-slate-300 px-3 py-1.5 rounded-lg border border-slate-700 transition-colors shadow-sm focus:outline-none flex items-center gap-2 text-xs font-medium"
                title="Copy Code"
              >
                {isCopied ? (
                  <>
              ...
              <button
                ...
                className="bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-1.5 rounded-lg border border-indigo-500 transition-colors shadow-sm focus:outline-none flex items-center gap-2 text-xs font-medium"
                title="Download .py"
              >
          ...
          <textarea
            ...
            spellCheck="false"
            className="w-full bg-[#0a0a0f] text-[#e2e8f0] p-6 rounded-b-xl border-x border-b border-slate-800 overflow-y-auto text-sm font-mono leading-relaxed h-[500px] custom-scrollbar shadow-inner focus:outline-none focus:border-indigo-500/50 resize-y selection:bg-indigo-500/30"
          />
        </div>
      </div>
    </div>
