...
export const BraggVisualization: React.FC<BraggVisualizationProps> = ({ wavelength, twoTheta: initialTwoTheta }) => {
  ...
  return (
    <div className="bg-slate-900 rounded-2xl p-6 shadow-2xl border border-slate-800 relative overflow-hidden group">
      ...
      <div className="flex items-center justify-between mb-6 relative z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-white tracking-tight">Diffraction Visualizer</h2>
            <p className="text-xs text-slate-400">Interactive Bragg's Law demonstration</p>
          </div>
        </div>
        
        <div className="flex items-center gap-4 bg-slate-800/50 p-2 rounded-lg border border-white/5">
          <span className="text-xs text-slate-400 font-medium">2θ Adjust:</span>
          <input 
            type="range" 
            min="10" 
            max="80" 
            step="0.1"
            value={localTwoTheta}
            onChange={(e) => setLocalTwoTheta(parseFloat(e.target.value))}
            className="w-32 accent-indigo-500"
          />
          <span className="text-xs font-mono text-indigo-400 w-12 text-right">{localTwoTheta.toFixed(1)}°</span>
        </div>
      </div>

      <div className="w-full bg-slate-950 rounded-xl overflow-hidden border border-slate-800 relative shadow-inner">
        <div className="absolute top-4 left-4 text-white font-mono text-xs z-10 space-y-2 bg-slate-900/80 p-3 rounded-lg backdrop-blur-sm border border-white/10 shadow-lg">
          <div className="flex items-center gap-3">
            <span className="text-slate-500 w-4">θ</span>
            <span className="text-emerald-400 font-bold">{theta.toFixed(2)}°</span>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-slate-500 w-4">d</span>
            <span className="text-amber-400 font-bold">{dSpacing.toFixed(4)} Å</span>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-slate-500 w-4">λ</span>
        ...
        <svg width="100%" height="100%" viewBox={`0 0 ${width} ${height}`} className="pointer-events-none">
          ...
          {/* Atomic Planes */}
          <line x1="0" y1={topAtomY} x2={width} y2={topAtomY} stroke="rgba(255,255,255,0.05)" strokeWidth="1" strokeDasharray="4 4" />
          <line x1="0" y1={bottomAtomY} x2={width} y2={bottomAtomY} stroke="rgba(255,255,255,0.05)" strokeWidth="1" strokeDasharray="4 4" />
          ...
          <path d={`M ${centerX} ${topAtomY} L ${cx} ${cy} L ${centerX} ${bottomAtomY} Z`} fill="rgba(16, 185, 129, 0.1)" stroke="none" />
          <path d={`M ${centerX} ${topAtomY} L ${dx} ${dy} L ${centerX} ${bottomAtomY} Z`} fill="rgba(16, 185, 129, 0.1)" stroke="none" />
          ...
          <text x={(centerX + cx)/2 - 20} y={(topAtomY + cy)/2} fill="#10b981" fontSize="10" fontWeight="bold">d sinθ</text>
          <text x={(centerX + dx)/2 + 5} y={(topAtomY + dy)/2} fill="#10b981" fontSize="10" fontWeight="bold">d sinθ</text>

          {/* Rays - Base Lines */}
          <line x1={inc1StartX} y1={inc1StartY} x2={inc1EndX} y2={inc1EndY} stroke="rgba(99, 102, 241, 0.15)" strokeWidth="4" />
          <line x1={ref1StartX} y1={ref1StartY} x2={ref1EndX} y2={ref1EndY} stroke="rgba(99, 102, 241, 0.15)" strokeWidth="4" />
          <line x1={inc2StartX} y1={inc2StartY} x2={inc2EndX} y2={inc2EndY} stroke="rgba(99, 102, 241, 0.15)" strokeWidth="4" />
          <line x1={ref2StartX} y1={ref2StartY} x2={ref2EndX} y2={ref2EndY} stroke="rgba(99, 102, 241, 0.15)" strokeWidth="4" />
          ...
          {/* Angle Markers */}
          <path d={`M ${centerX - 40} ${topAtomY} A 40 40 0 0 0 ${centerX - 40 * Math.cos(thetaRad)} ${topAtomY - 40 * Math.sin(thetaRad)}`} stroke="rgba(255,255,255,0.3)" fill="none" strokeDasharray="2 2" />
          <text x={centerX - 55} y={topAtomY - 15} fill="rgba(255,255,255,0.7)" fontSize="12">θ</text>
